...
class Auth_Controller extends Template_Controller {
	...
	public function login()
	{
		...
		if ($post->validate())
		{
			$this->user = $user = $this->load_user($post['username'], $post['password']);
	...
	private function load_user( $username, $password ) {
		...
		$ldap_config = Kohana::config( 'admin.ldap' );
		if( ! $user->loaded 
			&& array_key_exists('enabled', $ldap_config) 
			&& $ldap_config['enabled'] ) 
		{
			$ldap_host = $ldap_config['ldap_host'];
			$ldaprdn  =  $ldap_config['ldap_rdn']; 
			$ldappass =  $ldap_config['ldap_pass'];
